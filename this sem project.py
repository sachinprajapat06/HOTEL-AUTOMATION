...
def reception():
    f("welcome to the reception sir how can i help you")
    while True:
        query = takeCommand().lower()
        if 'waiter' in query:
...
def waiter():
    ...
    while True:
        ...
        if 'reception' in query:
            ...
            m("you must go straight and there is security")
            sequrity()
        elif 'show menu' in query or 'menu dekhao' in query or 'list dekhaiye' in query:
            m("ok sir here we are.")
            im = cv2.imread("restaurant.jpg")
            cv2.imshow("restaurant.jpg", im)
            cv2.waitKey(0)
        elif 'chinese' in query or 'chinese khana' in query:
            m("ok sir here we are.")
            im = cv2.imread("chinese.jpg")
            cv2.imshow("chinese.jpg", im)
            cv2.waitKey(0)
        elif 'desi menu' in query or 'desi menu dekhao' in query or 'desi khana' in query or 'indian' in query:
            m("ok sir here we are.")
